(async()=>{
    ...
    try{
        if(!location.href.includes("root=createConnectJob"))throw new Error("Open /data/?root=createConnectJob before running this bookmark.");
    ...
    await chooseSearch("basicSelect|input",C.template);
    await clickEnabled("oj-button.button-continue button","Template Continue");
    await sleep(3000);
    await wait("oj-list-view#fieldMappingList ul[role='grid'][aria-label='FieldMappingData']","field mapping table");
    await wait("oj-list-view#fieldMappingList li[role='row']","field mapping rows");
    await renameSourceObject();
