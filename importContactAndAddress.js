...
const createSourceObjectAndUploadSample=async()=>{
  ...
  const csvContent=`sourcecustomerid,sourcecontactpointid,sourceaddressid,email,mobilephone,country,state,city,zipcode,firstname,lastname,gender,birthdate,jobtitle,optinstatus,isdeliverable
1,cp-1,add-1,jagan.test01@yahoo.com,16504522260,US,Colorado,Denver,80201,Sachin,Tendulkar,M,07/10/26,Sales Executive,In,TRUE
`;
  ...
  if(typeof window.runCdpContactAndAddressFieldMapping!=="function"){
    throw new Error("cdpContactAndAddressFieldMapping.js was not loaded.");
  }

  await window.runCdpContactAndAddressFieldMapping();
};
