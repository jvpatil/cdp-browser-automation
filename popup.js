...
function jobPresetConfig(kind) {
  ...
  if (kind === "responsys") return { ...common, kind, sourceName: state.responsysSourceName, schedule: cloneConfig(schedulerConfig("responsys")) };
...
function flowPresetConfig() {
  return {
    ...
    responsysSourceName: state.responsysSourceName,
...
function applyPresetConfig(config) {
  ...
  if (config.kind === "responsys") { state.schedulers.responsys = normalizeScheduler("responsys", config.schedule); state.responsysSourceName = String(config.sourceName || state.responsysSourceName || "Responsys").trim() || "Responsys"; }
  ...
  if (!config.kind && Array.isArray(config.steps)) {
    state.responsysSourceName = String(config.responsysSourceName || state.responsysSourceName || "Responsys").trim() || "Responsys";
    ...
    state.schedulers.flowResponsys = normalizeScheduler("flowResponsys", config.responsysSchedule);
...
function renderSchedulers() {
  ...
  document.getElementById("flowResponsysScheduleLabel").textContent = schedulerSummary(state.schedulers.flowResponsys);
}
...
function renderFlow() {
  const steps = selectedFlowSteps(); const hasJobs = steps.includes("export") || steps.includes("import") || steps.includes("responsys");
...
async function send(message, label, runInfo) { try { const isResponsysRun = message.type === "run-task" && message.filename === "importJob.js"; const customTransferStep = message.type === "run-custom-flow" && message.flow.steps.some((step) => ["source", "destination", "export", "import"].includes(step)); const needsTemplate = (! ["run-publish-all", "run-data-viewer", "run-data-model", "run-data-model-attributes", "run-custom-flow"].includes(message.type) && !isResponsysRun) || customTransferStep; if (needsTemplate && !state.templateId) throw new Error("Select a transfer template first."); const tabId = await activeTab(); const status = `Running: ${label}`; const pendingRun = { ...(runInfo || { summary: label, details: label }), startedAt: Date.now(), logs: [{ at: Date.now(), level: "info", message: `Started: ${label}` }] }; await chrome.storage.local.set({ pendingRun, e2eStatus: status, ...(needsTemplate ? { lastTemplateId: state.templateId } : {}) }); setStatus(status); chrome.runtime.sendMessage({ ...message, tabId, connectionPurpose: state.connectionPurpose.trim(), ...(needsTemplate ? { templateId: state.templateId } : {}) }); closeAfterLaunch(); } catch (error) { setStatus(`Failed: ${error.message || error}`); } }
...
document.getElementById("runE2EBtn").addEventListener("click", () => send({ type: "run-full-sequence", responsysSourceName: state.responsysSourceName }, "Sanity Flow", { summary: "Sanity Flow — Data Models → Data Viewer → Source → Destination → Export → Import → Responsys", details: "Sanity Flow — Data Models → Data Viewer → Source → Destination → Export → Import → Responsys → Publish → Verify" }));
...
document.getElementById("runResponsysBtn").addEventListener("click", () => { const schedule = schedulerConfig("responsys"); send({ type: "run-task", filename: "importJob.js", schedule, responsysSourceName: state.responsysSourceName }, "Import Responsys Profile", { summary: `Responsys Import — ${scheduleSummary(schedule)}`, details: `Import Responsys Profile — ${scheduleSummary(schedule)}` }); });
...
["responsysSourceName", "flowResponsysSourceName"].forEach((id) => {
  document.getElementById(id).addEventListener("input", (event) => {
    state.responsysSourceName = event.target.value;
    document.getElementById(id === "responsysSourceName" ? "flowResponsysSourceName" : "responsysSourceName").value = state.responsysSourceName;
    chrome.storage.local.set({ lastResponsysSourceName: state.responsysSourceName });
    persistDraft();
  });
});
...
async function initialize() {
  try {
    ...
    const { flowDraft, e2eStatus, lastRun, runHistory, pendingRun, popupView, lastConnectionPurpose, lastResponsysSourceName, lastDataModelAttributeGroup } = await chrome.storage.local.get({ flowDraft: null, e2eStatus: "", lastRun: null, runHistory: [], pendingRun: null, popupView: "run", lastConnectionPurpose: "", lastResponsysSourceName: "Responsys", lastDataModelAttributeGroup: "Profile" });
    ...
    state.responsysSourceName = String(flowDraft?.responsysSourceName || lastResponsysSourceName || "Responsys").trim() || "Responsys";
    ...
    document.getElementById("responsysSourceName").value = state.responsysSourceName;
    document.getElementById("flowResponsysSourceName").value = state.responsysSourceName;
